    ...
    if not registro_activo:
        ...
        if df_escuelas.empty:
            ...
            if st.button("Confirmar y Registrar Agenda", disabled=not formulario_listo):
                ...
                datos_reserva = {
                    "CUE": cue_ingresado,
                    # Bloque de validación que debes insertar
    if cue_ingresado:
        cue_limpio = normalizar_texto(cue_ingresado)
        # Obtenemos las reservas actuales (debes tener la función obtener_reservas_df definida)
        df_reservas_actual = obtener_reservas_df()
        cues_registrados = df_reservas_actual['CUE'].astype(str).tolist() if 'CUE' in df_reservas_actual.columns else []
        
        if cue_limpio in cues_registrados:
            st.error(f"❌ La escuela con CUE {cue_limpio} YA tiene una reserva.")
            escuela_valida = False # Esto detiene el flujo
        else:
            # Aquí pegas tu lógica original que busca la escuela en df_escuelas
            coincidencia_esc = df_escuelas[df_escuelas['CUE'] == cue_limpio]
